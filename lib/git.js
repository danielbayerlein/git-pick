...
function isCommitAvailable(commit) {
  return shell.exec(`git cat-file -t ${commit}`) === 'commit';
}
...
function isCommitMergeable(commit) {
  return !shell.exec(`git format-patch -1 ${commit} --stdout | git apply --check -`);
}
...
function isWorkingDirectoryClean() {
  return !shell.exec('git status --porcelain');
}
